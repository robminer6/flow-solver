...
function isEdge(grid: FlowSpace[][], i: number, j: number) {
    ...
    if (grid[i - 1][j] !== FlowSpace.Empty) {
        // Space above isn't empty, lets see if it counts as an edge piece
...
export default class FlowGame {
    ...
    constructor(arr?: string[][]) {
        if (arr) {
            // Constructs grid based on parameter
            this.grid = [];
    ...
    solveCircleRecurse(
        ...
        if (i !== 0 && lastspace[0] !== i - 1) {
            ...
            if (
                isEdge(this.grid, i - 1, j) &&
                (this.grid[i - 1][j] === FlowSpace.Empty ||
                    this.grid[i - 1][j] === circle)
            ) {
        ...
        if (j !== 0 && lastspace[1] !== j - 1) {
            // Check space left.
            if (
                isEdge(this.grid, i, j - 1) &&
                (this.grid[i][j - 1] === FlowSpace.Empty ||
                    this.grid[i][j - 1] === circle)
            ) {
                potentialPath.push([i, j]);
                return this.solveCircleRecurse(
